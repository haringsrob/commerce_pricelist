...
class CSVFileObject extends \SplFileObject {
  ...
  public function valid() {
    ...
    if ($valid) {
      // The file is not valid if the column names could not be mapped.
      return !empty($this->columnNames);
    }
  ...
  public function setColumnNames(array $column_names) {
    $this->seek($this->getHeaderRowCount() - 1);
    // Flip the array to determine column key to column name mapping.
    $header_row = array_flip($this->current());
    foreach ($column_names as $column_name) {
      if (isset($header_row[$column_name])) {
        $this->columnNames[$header_row[$column_name]] = $column_name;
      }
    }
    $this->rewind();
  }
